...
#iconv -f ISO-8859-1 -t US-ASCII//TRANSLIT REVALO_AGRE_20191027224502.CLIP.err -o REVALO_AGRE_20191027224502.CLIP.err_USASCII
...
#concat="LANG=fr_FR.iso88591 sed \""

# Use of AWK instead of SED to solve encoding and special characters problems.
concat="awk '"
...
if [ "${old_val}" = "330-40002A" ]; then
   concat="${concat}/${condition}/ { gsub(/${old_val}/, \"${new_val} \")}; "
   else
   concat="${concat}/${condition}/ { gsub(/${old_val}/, \"${new_val}\")}; "
fi
 ...
 concat="${concat} {print}' $TMP_STR1 > $TMP_STR3"
  echo "$concat"

  # This line will execute the "sed/awk" command stocked in the "concat" variable.
  #eval "$concat"
}
